 ...
 * exist in the provider. -EINVAL when the name can't be found. NULL or a valid
						     ...
						     int index)
{
	...
	if (index >= core->num_parents || !core->parents)
				  ...
				  struct clk_core *parent)
{
	...
	for (i = 0; i < core->num_parents; i++) {
		...
		if (!strcmp(parent->name, core->parents[i].fallback)) {
	...
	for (i = 0, parent = parents; i < num_parents; i++, parent++) {
		if (parent_names) {
			...
			WARN(!parent_names[i],
				...
				__func__, core->name);
			ret = clk_cpy_name(parent->fallback, parent_names[i],
					   true);
		} else if (parent_data) {
			parent->hw = parent_data[i].hw;
			ret = clk_cpy_name(parent->name, parent_data[i].name,
					   false);
			if (!ret)
				ret = clk_cpy_name(parent->fallback,
						   parent_data[i].fallback,
						   false);
		} else if (parent_hws) {
