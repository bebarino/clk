...
struct drm_dp_typec_bridge_data {
	...
	size_t num_lanes;
	struct drm_dp_typec_bridge_typec_port *ports;
...
devm_drm_dp_typec_bridge_alloc(struct device *parent, const struct drm_dp_typec_bridge_desc *desc)
{
	...
	struct device_node *np = desc->of_node;
	struct device_node *dp_ep __free(device_node) = NULL;
	struct device_node *typec_ep;
				      ...
				      struct device_node *np)
{
	...
	struct drm_dp_typec_bridge_typec_port *ports, *port;
	...
	for_each_endpoint_of_node(np, typec_ep) {
		...
		port->mode_switch = typec_mux_register(dev, &mode_switch_desc);
