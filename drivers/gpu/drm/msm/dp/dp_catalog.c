...
{
	//pr_info("%s %#x and %#x\n", __func__, offset, readl(catalog->io->phy_io.base + 0x18));
...
static inline void dp_write_aux(struct dp_catalog_private *catalog,
			       u32 offset, u32 data)
{
	//pr_info("%s %#x %#x and %#x\n", __func__, offset, data, readl(catalog->io->phy_io.base + 0x18));
	/*
...
void dump_regs(void * __iomem *base, int len)
{
	...
	for(i = 0; i < len; i += 16) {
		...
		pr_info("%08x: %08x %08x %08x %08x", base, x0, x4, x8, xc);
		base += 16;
	}
}
...
void dp_catalog_dump_regs(struct dp_catalog *dp_catalog)
{
	...
	pr_info("AHB regs\n");
	dump_regs(catalog->io->dp_ahb.base, catalog->io->dp_ahb.len);

	pr_info("AUXCLK regs\n");
	dump_regs(catalog->io->dp_aux.base, catalog->io->dp_aux.len);

	pr_info("LCLK regs\n");
	dump_regs(catalog->io->dp_link.base, catalog->io->dp_link.len);
...
bool dp_catalog_ctrl_mainlink_ready(struct dp_catalog *dp_catalog)
{
	...
	/* Poll for mainlink ready status */
	ret = readl_poll_timeout(catalog->io->dp_link.base +
				REG_DP_MAINLINK_READY,
